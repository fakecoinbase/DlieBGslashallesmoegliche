...
class Values:
    ...
    def set(self, key, value):
        updater = http.client.HTTPConnection('http://192.168.0.3', 8080, timeout=20)
        updater.request("GET", "/set?"+str(key)+"="+str(value))
        updater.close()      
...
class Client:
    def __init__(self):
        connection = http.client.HTTPConnection('192.168.0.3', 8080, timeout=20)
        while True:
            connection.request("GET", "/get")
            values.values = ast.literal_eval(connection.getresponse())
            sleep(2)
